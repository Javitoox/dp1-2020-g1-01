...
public interface PagoRepository extends CrudRepository<Pago, Integer> {
	...
	@Query("SELECT a FROM Pago p JOIN p.alumnos a WHERE a.nickUsuario = p.alumnos.nickUsuario AND p.concepto = :concepto AND a.fechaMatriculacion > '1980-01-01' and a.fechaBaja IS null")
	...
	@Query("SELECT a FROM Alumno a WHERE a.nickUsuario IN (SELECT a.nickUsuario FROM Alumno a WHERE NOT EXISTS"
			+ " (SELECT p.alumnos.nickUsuario FROM Pago p where a.nickUsuario=p.alumnos.nickUsuario AND p.concepto = :concepto)) AND a.fechaMatriculacion > '1980-01-01' and a.fechaBaja IS null ")
	...
	@Query("SELECT a.nickUsuario FROM Alumno a WHERE a.nickUsuario IN ( SELECT p.alumnos.nickUsuario FROM Pago p GROUP BY p.alumnos.nickUsuario "
			...
			+ "AND a.fechaMatriculacion > '1980-01-01' and a.fechaBaja IS null)")
